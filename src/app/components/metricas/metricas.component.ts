...
export class MetricasComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.metricasActividad = {
      id_metrica: 0, id_actividad: 0, nota_quiz: 0, nota_evaluacion: 0,
      ...
      this.AuthDService.loadAllEvento().subscribe(res => {
        ...
        if (this.eventos.length) {
          for (let n = 0; n < this.eventos.length; n++) {
            ...
            if (this.eventos[n].oculto != 1) {
              if (this.eventos[n].id_actividad == this.actividadToSave.id_actividad) {
                ...
                this.metricas.push({
                  id_metrica: this.eventos[n].id_evento, id_evento: this.eventos[n].id_evento,
          ...
          for (let y = 0; y < this.metricas.length; y++) {
            ...
            this.metricasVisualizar.push({
              id_metrica: this.metricas[y].id_evento, id_evento: this.metricas[y].id_evento,
